...
import com.example.submissionfundamentalandroid2.databinding.FragmentFollowersBinding
...
class FollowersFragment : Fragment() {
    ...
    override fun onCreateView(
        ...
        viewModel.followers.observe(viewLifecycleOwner, Observer { followers ->
            setUserData(followers)
        })

        viewModel.followersLoading.observe(viewLifecycleOwner, Observer {
            showLoading(it)
        })
    ...
    private fun setUserData(users: List<UserData>) {
        if(users.isNotEmpty()){
            ...
            Log.e(ContentValues.TAG, users.toString())
